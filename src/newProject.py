...
noise = PerlinNoise(octaves=2, seed=2317)
print(randseed)
...
class Board:
    ...
    def __init__(self):        
        ...
        for x in range(1+width//self.sizeSquare):
            self.squares.append([])
            for y in range(1+height//self.sizeSquare):
    ...
    def update(self):
        self.flipper += 1
        randRange = 50
        if len(self.particles) < 7000:
            for x in range(50):
                newX = random.randint(0,width)
                newY = random.randint(0,height)
                self.particles.append(Particle(newX,newY))
